...
contract YulDeployer is Test {
    ...
    function deployContract(string memory fileName) public returns (address) {
        string memory bashCommand = string.concat('cast abi-encode "f(bytes)" $(solc --evm-version=london --strict-assembly --optimize yul/', string.concat(fileName, ".yul --bin | tail -1)"));
